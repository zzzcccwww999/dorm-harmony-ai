...
| 目标技术路线 | Vue + Python FastAPI + LangChain + SQLite / JSON；当前已落地 FastAPI + 规则评分模型 + LangChain/OpenAI 后端 AI 接口 |
| 目标核心流程 | 宿舍事件记录 -> 压力值分析 -> 冲突风险趋势提示 -> AI 多角色沟通模拟 -> 沟通复盘报告；当前已实现后端健康检查、压力分析、沟通模拟和沟通复盘接口，前端完整联调仍属后续范围 |
...
| AI 多角色沟通模拟 | 第二阶段后端 AI 已实现 | 后端通过 LangChain/OpenAI 生成直接型、回避型、调和型虚拟舍友结构化回复；前端 AI 页面联调仍属曹乐任务或后续范围 |
| 沟通复盘报告 | 第二阶段后端 AI 已实现 | 后端通过 LangChain/OpenAI 分析用户话术，输出表达优点、潜在问题、优化话术和后续行动建议；前端展示联调仍属后续范围 |
...
项目目标采用前后端分离架构。当前第二阶段后端已落地 FastAPI 接口层、压力评分模块、安全提示逻辑、LangChain Prompt 和 OpenAI 结构化调用；持久化历史记录、前端 AI 页面联调和完整 Demo 仍为后续范围。

```text
用户浏览器
  -> Vue 前端页面
  -> Python FastAPI 后端
  -> 压力评分模块 / LangChain AI 模块 / 数据存储模块（计划）
  -> OpenAI 模型 API / SQLite 或 JSON 文件（计划）
```

第二阶段当前后端运行范围为 FastAPI + 规则评分模型 + LangChain/OpenAI AI 服务：已实现 `/health`、`/api/analyze`、`/api/simulate` 和 `/api/review`，尚未实现历史记录存储与查询，也尚未完成前端 AI 页面联调和完整 Demo。
...
| 模块 | 当前状态 | 说明 |
| --- | --- | --- |
| API 接口层 | 第二阶段后端已实现核心接口 | 已提供 `/health`、`/api/analyze`、`/api/simulate` 和 `/api/review`，返回结构化 JSON 数据 |
...
当前后端已实现接口为 `/health`、`/api/analyze`、`/api/simulate` 和 `/api/review`；`/api/history`、前端 AI 页面联调和完整 Demo 不属于本次第二阶段后端 AI 文档任务的已完成范围。
...
以下内容为第二阶段后端 AI 已实现能力：`/api/simulate` 和 `/api/review` 通过 LangChain 调用 OpenAI 模型，并返回便于前端展示的结构化数据。前端 AI 页面联调、完整 Demo、历史记录存储、第 7 天演示视频和宣传海报仍不在本阶段后端范围内。
...
| 网页 Demo | 目标可演示事件记录、压力分析、AI 沟通模拟、沟通复盘报告；当前第二阶段已完成后端 AI 接口，完整前端联调 Demo 仍属后续范围 | 朱春雯 + 曹乐 |
